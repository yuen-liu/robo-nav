...
def load_images(image_folder=None, video_path=None, fps=10, image_ext=".jpg,.png,.JPG",
    ...
    if video_path is not None:
        ...
        ext_list = [e.strip() for e in image_ext.split(",") if e.strip()]
        normalized_exts = tuple(e.lower() if e.startswith(".") else f".{e.lower()}" for e in ext_list)
        if ".jpg" in normalized_exts and ".jpeg" not in normalized_exts:
            normalized_exts = normalized_exts + (".jpeg",)
        paths = []
        for root, _, files in os.walk(image_folder):
            for f in files:
                if f.lower().endswith(normalized_exts):
