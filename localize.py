...
Pipeline Stages:
1. Stage 1 (GPU Batched DINOv2 Coarse Search): Rapid visual similarity keyframe retrieval.
2. Stage 2 (Local Subsequence 6DoF Pose Transform): Computes local relative SE(3) pose delta T.
3. Stage 3 (Full Map Reconstruction + Metric Scale Calibration):
   Reconstructs the full map, solves scale scalar alpha (converting predictions to meters),
   and places the query image at its exact 6DoF pose in global metric space.
"""
...
def localize_query(
    ...
    Returns:
        Dict containing query filepath, anchor keyframe path, 3D metric position,
        3x3 rotation matrix, 4x4 SE(3) c2w matrix, and solved metric scale alpha.
    """
    if use_sdpa is None:
    ...
    # ── STAGE 2: Local Subsequence 6DoF Pose Inference ────────────────────────
    # Construct a contiguous keyframe window around the best match + query image
    window_radius = 2
    ...
    # Run streaming geometry model to extract joint camera poses for the local window
    ...
    # Convert pose encodings to 4x4 SE(3) camera-to-world (c2w) transformation matrices
    ...
    # ── STAGE 3: Full Map Reconstruction + Metric Scale Calibration ───────────
    ...
    # Compute depth scale ratio between Stage 3 (full map) and Stage 2 (local subsequence)
    ...
    # Scale the translation component of delta_T by the relative scale factor
    delta_T_scaled = delta_T.copy()
    delta_T_scaled[:3, 3] = delta_T[:3, 3] * rel_scale_factor

    # Compose final 6DoF pose in global map coordinate space: T_query = T_anchor @ delta_T_scaled
...
if __name__ == "__main__":
    ...
    parser.add_argument("--top_k", type=int, default=4, help="Number of top visual candidates to retrieve")
